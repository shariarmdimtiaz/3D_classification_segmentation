...
class TD3DInstanceHead(BaseModule):
    ...
    def _forward_second(self, x, targets, bbox_list):
        ...
        tensors, ids, rois, scores, labels = self.roi_extractor.extract([feats_with_targets], levels, rois, scores,
                                                                        labels)
    ...
    def forward_train(self,
        ...
        for i in range(len(bbox_list)):
            ...
            if len(boxes) != 0:
                ...
                sampled_boxes = img_metas[i]['box_type_3d'](boxes.tensor[idxs].view(-1, 7),
                                                            with_yaw=gt_bboxes[i].with_yaw)
        ...
        cls_preds, targets, v2r, r2scene, rois, scores, gt_idxs = self._forward_second(x[0], targets,
                                                                                       assigned_bbox_list)
    ...
    def _get_bboxes_single_test(self, bbox_preds, cls_preds, locations, cfg, img_meta):
        ...
        # print("\nbox, score, labels:", boxes, scores, labels)
    ...
    def _get_bboxes_test(self, bbox_preds, cls_preds, locations, cfg, img_metas):
        ...
        for i in range(len(img_metas)):
            ...
            if len(self.all_results) == 36:  # 한 번만 저장
                class_name = (
                    'aircon', 'ceiling', 'floor', 'wall', 'roundcolumn', 'column',
                    'window', 'door', 'stairs', 'slab', 'refrigerator',
                    'table', 'meetingtable', 'longtable', 'roundtable',
                    'computertable', 'chair', 'meetingchair', 'deskchair',
                    'cabinateA', 'cabinateB', 'cabinateC', 'board', 'monitor',
                    'desktop', 'keyboard', 'mouse', 'microoven', 'drawer',
                    'partition')

                boxes3d, scores, labels = result
                n = boxes3d.tensor.size(0)

                out_path = Path("data/inference_results/bbox/bbox.txt")
                out_path.parent.mkdir(parents=True, exist_ok=True)

                inst_cnt = defaultdict(int)  # 클래스별 instance 번호

                with out_path.open("w") as f:
                    for k in range(n):
                        lbl = int(labels[k])
                        name = class_name[lbl] if lbl < len(class_name) else f"class{lbl}"
                        inst = inst_cnt[lbl]
                        inst_cnt[lbl] += 1

                        x, y, z, dx, dy, dz = boxes3d.tensor[k, :6].tolist()
                        score = scores[k].item()
                        cz = z + (dz / 2)

                        # name instance score x y z dx dy dz
                        f.write(f"{name} {inst} {score:.4f} "
                                f"{x:.4f} {y:.4f} {cz:.4f} "
                                f"{dx:.4f} {dy:.4f} {dz:.4f} 0\n")

                print(f"[INFO] saved {n} boxes → {out_path}")

            #     all_path = out_path.parent / "all_results.txt"
            #     with all_path.open("w") as f:
            #         for frame_idx, (boxes3d_f, scores_f, labels_f) in enumerate(self.all_results):
            #             f.write(f"# frame {frame_idx}\n")  # ── 헤더
            #             inst_cnt = defaultdict(int)
            #
            #             for k in range(len(scores_f)):
            #                 lbl = int(labels_f[k])
            #                 name = class_name[lbl] if lbl < len(class_name) else f"class{lbl}"
            #                 inst = inst_cnt[lbl];
            #                 inst_cnt[lbl] += 1
            #
            #                 x, y, z, dx, dy, dz = boxes3d_f.tensor[k, :6].tolist()
            #                 cz = z + dz * 0.5
            #                 score = scores_f[k].item()
            #
            #                 # name instance score x y cz dx dy dz
            #                 f.write(f"{name} {inst} {score:.4f} "
            #                         f"{x:.4f} {y:.4f} {cz:.4f} "
            #                         f"{dx:.4f} {dy:.4f} {dz:.4f}\n")
            #
            #             f.write("\n")  # 프레임 구분용 빈 줄
            #
            #     print(f"[INFO] saved full list → {all_path}")
            # print("\n results : ", results)
        return results
    ...
    def forward_test(self, x, points, img_metas):
        ...
        return self._get_instances(cls_preds[:, -1], idxs[:, 0], v2r, r2scene, scores, labels, inverse_mapping,
                                   img_metas)
